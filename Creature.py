...
class Creature (Element):
    ...
    def reproduce (self, generate_creature=False):
        if (generate_creature):
            self.__elements[ElementType.CREATURE].add(self.new_creature(self.__elements, self._location.copy(), self.species))
        self.achieve_goal()
        self.__food.pop()
        self.__food.pop()
    ...
    def food_decision (self):
        ...
        if (self.health() < 0.1 and len(self.__food) > 0):
    ...
    def adjust_goal (self):
        ...
        if (self.__goal.peek() == Goal.REPRODUCE and len(self.__food) < 2 and self.health() > 0.70):
        ...
        if (self.__goal.peek() != Goal.HEAL and self.__path is None and self.is_moving() and self.health() < 0.20):
            self.__goal.clear()
            self.add_goal(Goal.REPRODUCE)
            self.add_goal(Goal.SHELTER)
        ...
        if (self.__goal.peek() == Goal.SHELTER and self.get_element_type(self.__curr_element) == ElementType.SHELTER):
            self.achieve_goal()
    ...
    def perform_action (self):
        ...
        # reproduce
        if (self.__goal.peek() == Goal.REPRODUCE and element_type == ElementType.SHELTER and self.creature_at_location() is not None and self.has_food(2)):
            creature = self.creature_at_location()
            if (creature.get_goal() == Goal.REPRODUCE and creature.has_food(2)):
                self.reproduce(generate_creature=True)
                self.achieve_goal()
                creature.reproduce(generate_creature=False)
                creature.achieve_goal()
    ...
    def creature_at_location (self):
        for element in get_location_dictionary(self.__elements[ElementType.CREATURE])[self._location]:
            if (isinstance(element, Creature) and element != self):
                return element
        return None

    # element at current location (barring the creature itself)
    ...
    def prepare_goal (self):
        ...
        if (self.__goal.peek() == Goal.HEAL):
            ...
            nearest = self.get_nearest_shelter()
    ...
    def __get_nearest (self, elements):
    ...
    def get_nearest_shelter (self):
        return self.__get_nearest(self.__elements[ElementType.SHELTER])
    ...
    def advance (self):
        ...
        if (self.__path is not None):
            self.__path_needed = False
        print(f"end of adv... food: {self.__food}")
